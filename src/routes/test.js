...
router.post("/store", (req, res) => {
  ...
  const {name, numQuestions} = req.body;
  const id = uuid.v1();
  const slug = slugify(req.body.name);
  const test = {id, slug, name, numQuestions}; 
  tests.push(test);
...
router.get("/:slug/question", (req, res) => {
  ...
  question = question.replace("{{slug}}", req.params.slug);
  html = html.replace("{{component}}", question);
  let test = tests.filter(test => test.slug === slug);
...
router.post("/:slug/question/store", async (req, res) => {
  let slug = req.params.slug;
  let tests = JSON.parse(fs.readFileSync('./src/data/test.json', 'utf-8'));
  ...
  let test = tests.map((test) => {
    if(slug === test.slug){
      if(test.questions === null || test.questions === "undefined"){
        test.questions = [];
      }else{
        test.questions.push({"oi3": "ola3"},{"pa4": "peeee4444"});
      }
    }
